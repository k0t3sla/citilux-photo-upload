  ...
  (doseq [file files]
    ...
    (let [size (fs/size file)]
      (cond
        ...
        (> 52428800 size) (do (sh/sh "ffmpeg" "-i" file "-threads" "1" "-fs" "18M" "-r" "30" "-vf" "scale=1080:1920" (str (:out-wb env) (create-path (get-article file)) (fs/name file) "_20.mp4"))
                              (copy-file file [(:out-wb env)]))
        :else (do (sh/sh "ffmpeg" "-i" file "-threads" "1" "-fs" "18M" "-r" "30" "-vf" "scale=1080:1920" (str (:out-wb env) (create-path (get-article file)) (fs/name file) "_20.mp4"))
